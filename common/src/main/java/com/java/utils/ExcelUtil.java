...
/**
 * TODO:EXCEL导出 create by :lu.xu  20170808
 */
public class ExcelUtil {
  ...
  /**
   * TODO:通用poi导出EXCEL 注意：该方法导出的以".xls" office2003版的excel文档，若以".xlsx"后缀命名文件将不能正常打开。若要使用"xlsx"后缀，则
   * 该方法中的"HSSFWorkbook类"换成"XSSFWorkbook", 相应的单元格组件、样式都需要改正
   *
   * @param sheetName sheet名称
   * @param headers 列头 key:属性名称 value：属性名称对应的列头中文名
   * @param dataSets 数据集
   * @param pattern 日期格式化格式
   * @return 生成的HSSFWorkbook
   */
  public static HSSFWorkbook exportExcel(String sheetName, LinkedHashMap<String, String> headers,
      Collection dataSets,
      ...
      while (it.hasNext()) {
        ...
        for (Map.Entry<String, String> header : headers.entrySet()) {
          ...
          if (EmptyUtils.isNotEmpty(textValue)) {
            ...
            boolean isMatch = Pattern.matches(doublePattern, textValue);
            if (isMatch) {
   ...
   * 创建单元格
   *
   * @param columnIndex 列索引-0开始
   * @param style 样式
   * @param value 值
   */
  public static void createCell(HSSFRow row, int columnIndex, HSSFCellStyle style, String value) {
