...
public class FileServiceImpl implements IFileService {
    ...
    public ResultMsg uploadFile(MultipartFile file, String accessToken) throws Exception {
        ...
        if (EmptyUtils.isNotEmpty(userId)) {
            ...
            return new ResultMsg(false, "无权访问");
        }
        if (file.isEmpty()) {
            return new ResultMsg(false, "文件为空");
        }
        /**
         * 文件名称乱码的话，see：
        ...
        if (index == -1) {
            return new ResultMsg(false, "文件必须存在后缀名");
        }
        ...
        return new ResultMsg(true, "上传成功", jsonObject);
    }
    ...
    public ResultMsg removeFile(String fileId, String accessToken) {
        ...
        if (fileInfo == null) {
            return new ResultMsg(false, "未查询到文件信息");
        }
        logger.info("fileInfo:" + fileInfo.getDirectory() + " " + fileInfo.getFileName() + fileInfo.getFileSuffix());
        ChannelSftp sftp = null;
        try {
            //开启sftp通道
            sftp = this.getChannelSftp();
            ...
            logger.error(e.getMessage());
            e.printStackTrace();
            return new ResultMsg(false, "删除文件异常");
        } finally {
            FTPUtil.closeChannel(sftp);
        }
    }
