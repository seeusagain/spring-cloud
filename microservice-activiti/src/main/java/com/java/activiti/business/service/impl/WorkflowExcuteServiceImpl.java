...
public class WorkflowExcuteServiceImpl implements IWorkflowExcuteService {
    ...
    public ResultMsg startWorkflow(String processDefinitionKey, String flowName, String businessKey,
        ...
        if (EmptyUtils.isAnyEmpty(processDefinitionKey, flowName, businessKey, accessToken)) {
            return new ResultMsg(false, "流程key、流程名称、业务主键、accessToken 必填");
        }
        logger
            .info(">>工作流开始：processDefinitionKey：{}，flowName：{}，businessKey：{}，accessToken：{}", processDefinitionKey, flowName, businessKey, accessToken);
        String userId = stringRedisTemplate.opsForValue().get(accessToken);
        if (EmptyUtils.isAnyEmpty(userId)) {
            return new ResultMsg(false, "用户令牌失效");
        }
        ...
        try {
            ...
            return new ResultMsg(true, noticeMsg, jsonObject);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("开始一个工作流异常");
            return new ResultMsg(false, null, e.getMessage());
        }
    }
    ...
    public ResultMsg completeTask(String taskId, String opinion, String opinionRemarks,
        ...
        logger
            .info(">>工作流执行开始：taskId：{}，opinion：{}，opinionRemarks：{}，accessToken：{}", taskId, opinion, opinionRemarks, accessToken);
        ...
        if (rmg.isSuccess()) {
            ...
            //异步发送邮件
            WorkFlowExcuteMailSender mailSender = new WorkFlowExcuteMailSender(task.getProcessInstanceId());
    ...
    private ResultMsg completeTaskNormal(String taskId, HashMap<String, Object> variables,
        ...
        if (EmptyUtils.isEmpty(tasksAfter)) {
            csmFlowTaskRespository.updateByInstanceId(CsmFlowTaskStatusEnums.COMPLETED.getCode(), new Date(), task
                .getProcessInstanceId());
            String noticeMsg = "任务已执行，且工作流已经执行完毕";
            logger.info(noticeMsg);
            return new ResultMsg(true, noticeMsg);
        } else {
            csmFlowTaskRespository
                .updateByInstanceId(CsmFlowTaskStatusEnums.RUNNING.getCode(), new Date(), task.getProcessInstanceId());
        }
        ...
        for (ActRuTask afterTask : tasksAfter) {
            ...
            if (isNewTask) {
                ...
                this.setAssigneeForUsertask(afterTask.getId(), assigneeCode, workFlowAssignee.getAssignee());
            }
        }
    ...
    private void setAssigneeForUsertask(String taskId, String assigneeCode, String assigneeId) {
        logger.info(">>给userTask授权开始：taksid：{}，assigneeCode：{}，assigneeId：{}", taskId, assigneeCode, assigneeId);
        Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
        ...
        if (EmptyUtils.isEmpty(assigneeCode)) {
            assignees = csmActAssigneeRespository.queryByProcessKeyAndElementCode(processKey, activitiKey);
        } else {
        ...
        if (EmptyUtils.isNotEmpty(assignees)) {
            ...
            if (assignees.size() > 1) {
                logger.error(">> 工作流设置受理人，发现同一节点多个重复授权；processKey:{},activitiKey:{}", processKey, activitiKey);
            }
        } else {
     ...
     * @param createUserId
     */
    private void addApproveRecords(String taskId, String taskName, String opinion, String opinionRemarks,
        String createUserId) {
        try {
            logger
                .info("添加审批记录: taskId:{},opinion:{},opinionRemarks:{},createUserId:{}", taskId, opinion, opinionRemarks, createUserId);
            ...
            logger
                .error("添加审批记录异常,taskId:{},opinion:{},opinionRemarks:{},createUserId:{}", taskId, opinion, opinionRemarks, createUserId);
            e.printStackTrace();
        }
    }
}
