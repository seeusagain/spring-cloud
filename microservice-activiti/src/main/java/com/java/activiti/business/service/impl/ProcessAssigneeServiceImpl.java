...
public class ProcessAssigneeServiceImpl implements IProcessAssigneeService {
  ...
  public ResultMsg addAssigneeObject(CsmActAssigneeObject assigneeObject, String accessToken) {
    if (null == assigneeObject) {
      return ResultMsg.error("对象为空");
    }
    if (EmptyUtils.isEmpty(accessToken)) {
      return ResultMsg.error("accessToken为空");
    }
    String userId = stringRedisTemplate.opsForValue().get(accessToken);
    if (EmptyUtils
        .isAnyEmpty(assigneeObject.getAssigneeId(), assigneeObject.getAssigneeType(), assigneeObject
            .getAssigneeObjectId(), assigneeObject.getAssigneeObjectName())) {
      return ResultMsg.error("节点ID、授权类型、授权对象ID、名称 不能为空");
    }
  ...
  public ResultMsg queryProcessAssigneeList(String keywords, PageInfo pageInfo) {
    ...
    List<CsmActAssigneeEntity> results =
        this.csmActAssigneeMapper
            .queryDistinctList(keywords, pageInfo.getStartRecord(), pageInfo.getEndRecord());
    int total = this.csmActAssigneeMapper.countDistinctList(keywords);
    pageInfo.setTotal(total);
    pageInfo.setResultSets(results);
    return ResultMsg.ok(null, pageInfo);
  }

  @Override
  public ResultMsg queryElementList(String processKey) {
    List<CsmActAssignee> results = this.csmActAssigneeRespository
        .queryByProcessKeyOrderByElementCode(processKey);
    return ResultMsg.ok(null, results);
  }

  @Override
  public ResultMsg queryAssigneeList(String assigneeId) {
    List<CsmActAssigneeObject> results = this.csmActAssigneeObjectRespository
        .queryByAssigneeId(assigneeId);
    return ResultMsg.ok(null, results);
  }
      ...
      try {
        ...
        for (File f : files) {
          ...
          Document document = reader
              .read(new File(allProcessFilePath + File.separator + f.getName()));
