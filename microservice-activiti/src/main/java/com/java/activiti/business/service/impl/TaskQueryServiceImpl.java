...
public class TaskQueryServiceImpl implements ITaskQueryService {
  ...
  public InputStream queryProcess(String processInstanceId) throws Exception {
    ...
    InputStream imageStream = diagramGenerator
        .generateDiagram(bpmnModel, "png", highLightedActivitis, highLightedFlows, "宋体", "宋体", "宋体",
            null, 1.0);
    //        单独返回流程图，不高亮显示
  ...
  @Override
  public ResultMsg queryTaskList(String accessToken, String taskType, String taskStatus,
      String keywords,
      PageInfo pageInfo) {
    if (EmptyUtils.isAnyEmpty(accessToken, taskType)) {
      return ResultMsg.error("accessToken、taskType 参数必需");
    }
    ...
    if (WorkflowTaskTypeEnums.TASK_TYPE_TODO.getCode().equals(taskType)) {
      ...
      if (EmptyUtils.isNotEmpty(resultSets)) {
        total = workflowTaskQueryMapper
            .queryTodoCount(userId, CsmActAssigneeObjectTypeEnums.TYPE_USER
    ...
    //我参与的(只要是工作流节点涉及到当前用户的，都显示，不一定是只参与了审批)
      ...
      resultSets = workflowTaskQueryMapper
          .queryParticipated(userId, WorkflowVariablesEnums.VARIABLE_EXCUTEUSER
              .getCode(), keywords, taskStatus, pageInfo.getStartRecord(), pageInfo.getEndRecord());
      if (EmptyUtils.isNotEmpty(resultSets)) {
        total =
            workflowTaskQueryMapper
                .queryParticipatedCount(userId, WorkflowVariablesEnums.VARIABLE_EXCUTEUSER
    ...
    //我发起的
      ...
      resultSets = workflowTaskQueryMapper
          .queryStarted(userId, keywords, taskStatus, pageInfo.getStartRecord(),
              pageInfo.getEndRecord());
      if (EmptyUtils.isNotEmpty(resultSets)) {
        total = workflowTaskQueryMapper.queryStartedCount(userId, keywords, taskStatus);
      }
    } else {
      return ResultMsg.error("taskType参数不匹配");
    }
  ...
  public ResultMsg queryApproveUserInfo(String processInstanceId) {
    ...
    return ResultMsg.ok(null, objects);
  }

  @Override
  public ResultMsg queryApproveRecords(String processInstanceId) {
    if (EmptyUtils.isEmpty(processInstanceId)) {
      return ResultMsg.error("流程实例参数缺失");
    }
    //TODO：这块存在优化空间
    ...
    if (flowTask == null) {
      ...
      return ResultMsg.ok("数据为空");
    }
    //records
    List<QueryApproveRecordsEntity> results = queryApproveRecordsMapper
        .queryApproveRecords(processInstanceId);
