...
public class ModelCreateController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "modelName", value = "流程模型名称", required = true, paramType = "query"),
    ...
    public Object newModel(HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        if (EmptyUtils.isAnyEmpty(modelName, modelCode)) {
            return new ResultMsg(false, "参数必填 modelName:" + modelName + " modelCode:" + modelCode);
        }
    ...
    public Object editModel(HttpServletRequest request,
                            @RequestParam(value = "modelId", required = false) String modelId) throws UnsupportedEncodingException {
        //因为通过zuul访问，但是zuul不支持静态资源的返回，所以编辑界面直接打开非zuul访问地址
        String ip = HttpRequestUtil.getLocalIp();
        String finalUrl = "http://" + ip + ":" + port + contextPath + "/static/modeler.html?modelId=" + modelId;
        return new ResultMsg(true, "", finalUrl);
    }

    @ApiOperation(value = "获取所有工作流模板信息", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({@ApiImplicitParam(name = "keywords", value = "模板名称模糊查询", paramType = "query")})
    ...
    public List<Model> modelList(HttpServletRequest request,
        ...
        if (EmptyUtils.isEmpty(keywords)) {
            ...
            models = repositoryService.createModelQuery().modelNameLike("%"+keywords+"%").orderByCreateTime().desc().list();
        }
        return models;
    }
    ...
    public Object deleteModel(@PathVariable("id") String id) {
        ...
        if (model == null) {
            return new ResultMsg(false, "删除失败，未查询到相关模板");
        }
        repositoryService.deleteModel(id);
        return new ResultMsg(true, "删除模板成功");
    }
