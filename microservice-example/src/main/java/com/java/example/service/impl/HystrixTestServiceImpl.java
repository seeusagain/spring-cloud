...
public class HystrixTestServiceImpl implements IHystrixTestService {
    ...
    /**
     * hystrix当服务不可达时（sever停止或者 404路径），启用降级策略，调用本地方法
     * 注意，配置文件开启：
    ...
    public Object fallback() {
        ...
        ResultMsg resultMsg = this.authenticationControllerRemote.loginForFallback();
        return resultMsg;
    }
    ...
    public Object circuitBreakerTest() throws Exception {
        ...
        return new ResultMsg(true, "测试成功，请看服务器日志..");
    }
    
    /**
     * 熔断实现
     * 注意
     * 1.我们在配置文件中配置hystrix线程池为5，默认是10：hystrix:threadpool:default:coreSize: 5
     * 2.配置hystrix的超时时间：timeoutInMilliseconds,让被调用的组件线程等待市场小于熔断市场，保证测试
     *
     *
     * 结果：
     * 通过观察日志可以得出，当basic的借口睡眠3s情况下（小雨熔断配置6s）
     * 前5个线程（线程池配置，并发量=线程访问时间*线程池）访问basic接口后处于等待状态，同时线程池未释放，
     * 接着后五个线程由于获取不到空闲线程，访问了降级方法。
     * 降级方法访问5个后，此时触发熔断，报错了。
     * 断路器开条件：
     *  1.10s内请求失败数量达到20个，断路器开。
     *  2.出错百分比阈值，当达到此阈值后，开始短路。默认50%
     *
     * 资源隔离：
     * 当basic调用处于断路情况下，调用了example2组件，发现可以正常调用，说明资源进行了隔离
     * 同一个组件之间的调用会被熔断，但是不同的不会
     */
