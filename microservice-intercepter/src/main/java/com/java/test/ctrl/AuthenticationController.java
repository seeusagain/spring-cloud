...
/**
 * Created by lu.xu on 2018/7/2. TODO:用户登录、注销接口
 */
...
public class AuthenticationController {
  ...
  @ApiImplicitParams({
      @ApiImplicitParam(name = "userId", value = "账号", required = true, paramType = "query"),
  ...
  public ResultMsg login(HttpServletRequest request, UserInfo userInfo) {
    ...
    if (resultMsg.isSuccess()) {
      ...
      request.getSession()
          .setAttribute(AuthenticationConstats.USER_NAME_SESSION_KEY, logonUser.getDisplayName());
    }
    return resultMsg;
  }
